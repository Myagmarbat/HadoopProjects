...
public class Reduce extends Reducer<Text, MapWritable, Text, CustomMap> {

	@Override
	protected void reduce(Text item, Iterable<MapWritable> values,
			Reducer<Text, MapWritable, Text, CustomMap>.Context context)
		...
		for (Entry<Writable, Writable> entry : sumMap.entrySet()) {
			...
			resultMap.put(entry.getKey(), new DoubleWritable(Double.valueOf(df.format(r))));
		}
		context.write(item, resultMap);
	}
}
